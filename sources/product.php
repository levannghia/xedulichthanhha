<?php  
	...
	{
		/* Lấy cấp 1 detail */
		$row_detail = $pro_list = $d->rawQueryOne("select id, ten$lang, tenkhongdauvi, tenkhongdauen, type, photo, options,noidung$lang from #_product_list where id = ? and type = ? limit 0,1",array($idl,$type));
		$hinhanhsp_list = $d->rawQuery("select photo,tenvi from #_gallery where id_photo = ? and com='product' and type = ? and kind='man_list' and val = ? and hienthi > 0 order by stt,id desc",array($pro_list['id'],'san-pham','san-pham'));
		/* SEO */
		...
		$curPage = $get_page;
		$per_page = $optsetting['sanpham']>0 ? $optsetting['sanpham']: 20;
		$startpoint = ($curPage * $per_page) - $per_page;
		$limit = " limit ".$startpoint.",".$per_page;
		$sql = "select photo, ten$lang, tenkhongdauvi, tenkhongdauen, giamoi, gia, giakm, id,masp from #_product where $where order by stt,id desc $limit";
		$product = $d->rawQuery($sql,$params);
